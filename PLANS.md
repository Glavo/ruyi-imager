...
- GUI 已完成 MaterialFX 主界面、目录/本地镜像二选一流程、渐进启用、树形 OS 分类、搜索弹窗、i18n、首次安全提醒、目标确认、窗口图标和页头 Logo；刷写期间会折叠选择流程，仅保留当前制造商、开发板、镜像和目标摘要栏，并按后端 `ProgressEvent.stage` 分阶段显示下载、准备、写入、校验和 fastboot 等多个进度条。
