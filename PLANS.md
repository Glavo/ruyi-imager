...
- Windows UAC、Linux `pkexec`、macOS `osascript` 提权路径已接入；Windows UAC 启动失败会抑制 PowerShell progress CLIXML 并返回普通错误文本；已挂载 Windows 目标会通过 UAC 提权使用 `mountvol` 移除盘符/挂载目录访问路径并卸载卷，跳过不能移除的 volume GUID path，准备失败会提前报错而不是静默继续写入；已挂载目标会按平台能力进行准备或拒绝，并显示挂载点。
