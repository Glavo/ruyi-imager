...
- JLink packaging：`app` 模块提供 `jlinkRuntime`、`installJlinkDist` 和 `jlinkZip` 任务，生成当前平台裁剪 runtime、classpath 应用库、启动脚本以及 bundled fastboot/dd-flasher；可通过 `-Pjlink.modules=<modules>` 覆盖默认 runtime 模块集合。
