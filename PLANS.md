...
- Windows UAC、Linux `pkexec`、macOS `osascript` 提权路径已接入；Windows UAC 启动失败会抑制 PowerShell progress CLIXML 并返回普通错误文本；已挂载 Windows 目标会通过 UAC 提权卸载卷并移除访问路径，准备失败会提前报错而不是静默继续写入；已挂载目标会按平台能力进行准备或拒绝，并显示挂载点。
