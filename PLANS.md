...
- GUI supports catalog/local image selection, automatic metadata update before first manufacturer selection, target selection, safety confirmation, progress, cancellation, log path display, a dedicated language settings dialog, Chinese vendor display names, window icons, and short progress status text without trailing full stops.
