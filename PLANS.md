...
- Bundled dd-flasher：发行包会携带当前平台 Rust `dd-flasher` helper；运行时优先使用 `ruyi.imager.ddFlasher.executable` / `RUYI_IMAGER_DD_FLASHER` 配置，其次使用发行目录 bundled helper，最后退回 PATH；平台识别和本机打包支持 riscv64 / riscv64gc。
