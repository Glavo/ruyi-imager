...
- GUI 已实现 MaterialFX 风格主窗口、更宽更高的默认窗口、无阴影步骤控件和选择列表、移除底部提示、运行时中英文切换、语言偏好持久化、目录镜像/本地镜像二选一流程且左右标题居中、“或”分隔符垂直居中、本地镜像卡片为等高纵向布局、渐进式步骤启用、树形操作系统分类选择、dd/fastboot 目标切换、多分区 `dd-v1` 目标映射流程、策略/缓存/目标风险状态标记、结构化最终确认弹窗。
